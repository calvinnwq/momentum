...
export function emitStatusFailure(
  parsed: { json: boolean },
  io: CliIo,
  failure: {
    code: string;
...
export function emitStatus(
  ...
  const payload = {
    ...
    ...(data.pendingUpdateIntents.length > 0
      ? {
          ...
          pendingUpdateIntents: data.pendingUpdateIntents.map((intent) =>
            goalStatusPendingIntentToJsonShape(intent)
          ),
  ...
  if (data.pendingUpdateIntents.length > 0) {
    ...
    const countLabel = data.truncatedPendingUpdateIntents
      ...
      : `${shownCount}`;
    lines.push(`Pending update intents: ${countLabel}${staleSuffix}`);
...
export function goalStatusPendingIntentToJsonShape(
...
export function goalStatusPendingIntentExternalApplyToJsonShape(
...
export function goalStatusExternalApplyToJsonShape(
...
export function emitLogs(
  ...
  if (data.runnerLog.error !== undefined) {
    ...
    lines.push(
      data.runnerLog.content.endsWith("\n")
  ...
  if (data.verificationLog.error !== undefined) {
    ...
    data.verificationLog.exists &&
    data.verificationLog.content.length > 0
  ) {
    lines.push(
      data.verificationLog.content.endsWith("\n")
  ...
  if (data.resultJson.error !== undefined) {
    ...
    lines.push(
      data.resultJson.content.endsWith("\n")
...
export function emitLogsFailure(
  parsed: { json: boolean },
  io: CliIo,
  failure: {
    code: string;
...
export function emitHandoffFailure(
  parsed: { json: boolean },
  io: CliIo,
  failure: {
    code: string;

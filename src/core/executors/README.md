...
`agent-loop/sdk.ts` and `single-shot/sdk.ts` are the native binding-backed built-ins: `agent-loop`, `agent-once`, and `script` implement the same `Executor` interface and accept narrow runner adapters as lifecycle extension points.
...
The binding-backed host writes its step-scoped receipt before no-mistakes launch and before delegated reset or commit mutation.
...
Finalized binding-backed state is bound to the repository's current full `HEAD`, and cached terminal handoff proof still requires a fresh clean read for the same run, branch, and exact head before settlement.
...
Binding-backed recovery may take over an active lock for the same interrupted step-scoped dispatch correlation after lock expiry or after the scheduler proves and releases the matching stale dispatch owner, with compare-and-swap fencing against concurrent or newer ownership.

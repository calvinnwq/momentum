...
Daemon domain. This folder owns the daemon and worker compatibility runtime: the
daemon loop, daemon run/status state, queue-job persistence, stale-lease/run
recovery, and the single worker-run entry point. It holds business/runtime
behavior only — loop control, persistence, and read-only status inspection — and
...
`daemon-runs.ts` → `runs.ts`, `daemon-status.ts` → `status.ts`); `queue-jobs.ts`,
`stale-recovery.ts`, and `worker-run.ts` kept their names.

...
Status, handoff, monitor, and logs expose the selected `route.steps` through the read-only compatibility projection, while native dispatch reads the frozen `workflow_steps.agent_config_json` row and maps it onto executor rounds, including retries and reattachments.

...
The module owns only the pure representation/validation half (no SQLite, no file system, no clock); status/handoff/logs text rendering and daemon executor consumption of `route.steps` are later slices.

...
For current coding definitions, that wrapper invocation is the no-mistakes handoff beneath `delegate-supervisor`; later ticks call the validated no-mistakes executable with `axi status --run <external-run-id>` and normalize the correlated run id, branch, full head SHA, findings, decisions, pull request, and CI state through `src/adapters/no-mistakes-tool-adapter.ts`.

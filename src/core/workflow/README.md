...
The resulting plan is a pure projection of the version-pinned built-in definition plus inputs, including `route.implementationEngine`, each step's optional portable config, and its effective agent config, so a later `start-coding` from the same inputs persists a matching run.

`route/coding.ts` is the pure keystone for native per-step coding route/config overrides: it validates, normalizes, reads, and shapes operator `harness`/`model`/`effort` selections per configurable coding step (`implementation`, `postflight`, `validate`, `merge-cleanup`) under a byte-stable `route.steps` compatibility namespace; canonical run state lives in `workflow_steps.agent_config_json`, alongside the canonical-only `workflow_run_coding_compatibility` implementation-engine and profile values, which the compatibility projection no longer emits.
...
Status, handoff, monitor, and logs expose the selected `route.steps` through the read-only compatibility projection, while native dispatch reads the frozen `workflow_steps.agent_config_json` row through the strict `route/canonical-agent-config.ts` reader and maps it onto executor rounds, including retries and reattachments.

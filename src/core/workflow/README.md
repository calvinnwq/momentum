...
Provider-specific model aliases in run-specific route overrides are normalized during the same pure route pass when enough context is present, so known Claude, Codex, and OpenCode aliases preview, persist, and dispatch the command-ready model string for that harness instead of the bare alias; definition-level agent config, unknown values, and non-agent harness/model values remain free-form.

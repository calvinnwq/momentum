...
For that source, `dispatch/persist.ts` resolves executors from the built-in definition recorded on the run by key and version rather than any persisted definition rows with the same key/version, so the native door remains stable even when generic definition starts are using persisted overrides.
...
`workflow_run_lineage` (no retired `route.subworkflow` compatibility projection is
...
compatibility.
Before spawning, the helper validates that the run-local config has only a `steps` object and canonical snake_case per-step keys, rejects malformed `env_allow` and unsafe or mismatched `result_file` values as setup failures, and writes no agent-result evidence for rejected configs.
The validate step also requires an explicit `runner_profile` with `interface: "axi"`, `stdin: "closed"`, `agent` set to one of `claude`, `codex`, `opencode`, or `rovodev`, `required_env` containing the selected agent's required environment (`HOME` and `PATH`, plus `CODEX_HOME` for Codex), and an absolute executable `agent_path`; missing profile fields, missing filtered environment, `agent=auto`, a missing/non-executable agent path, or a no-mistakes `agent` / `agent_path_override.<agent>` mismatch fail closed before the no-mistakes command is spawned.
...
For `no-mistakes`, the helper also normalizes upstream terminal-success evidence: `checks-passed`, or a still-monitoring upstream run with current clean pull request evidence and green or explicitly absent checks, becomes successful agent-result evidence only when current blockers, active findings, unresolved gates, dirty / draft pull request state, and non-successful checks are absent.
...
The resulting plan is a pure projection of the version-pinned built-in definition plus inputs, including each step's executor-owned portable config and its effective agent config, so a later `start-coding` from the same inputs persists a matching run.
The recorded definition key/version, matching versioned step executor, and frozen portable config are the active implementation authority; the compatibility label is read-back/refusal evidence only.

`route/coding.ts` is the pure keystone for native per-step coding agent-config overrides: it validates, normalizes, reads, and shapes operator `harness`/`model`/`effort` selections per configurable coding step (`implementation`, `postflight`, `validate`, `merge-cleanup`) in a byte-stable canonical order; canonical run state lives in `workflow_steps.agent_config_json`, alongside historical labels in the canonical-only `workflow_run_coding_compatibility` destination.
The `workflow run start-coding` / `workflow run preview-coding` doors accept an `--agent-config-json` flag (replacing the retired `--steps-json` flag) that builds overrides via this module and hands them to canonical persistence (or the frozen preview, which also projects a human-readable per-step selection block); the generic `workflow run start` refuses the flag with `route_config_not_allowed`, and a misconfigured selection fails closed with `route_config_invalid` before any write.
Provider-specific model aliases in the merged native step selection are normalized before preview/start persistence when enough context is present, so known Claude, Codex, and OpenCode aliases preview, persist, and dispatch as the command-ready model string for that harness instead of the bare alias; unknown values and non-agent harness/model values remain free-form.
Status, list, handoff, and logs expose the frozen selection as step-level `agentConfig` and read compatibility/lineage/import metadata through their direct typed canonical readers; the monitor path uses the same typed detail loader before deriving its compact envelope. Native dispatch reads the frozen `workflow_steps.agent_config_json` row through the strict `route/canonical-agent-config.ts` reader and maps it onto executor rounds, including retries and reattachments.
Live-wrapper execution forwards those mapped values as `MOMENTUM_AGENT_PROVIDER`, `MOMENTUM_MODEL`, and `MOMENTUM_EFFORT`; corrupt canonical step config fails closed to manual recovery before executor work instead of silently falling back.

`preflight/structural.ts` is the pure structural preflight seam for workflow setup.
The start and preview doors use it to validate built-in definition lookup, required run shape, approval boundary, issue scope, per-step agent config, registered executor presence, and portable config against each registered declaration before durable writes.
...
Status, handoff, monitor, and GUI readers remain future consumers until they are wired to the same executor-round projection instead of agent-authored JSON, terminal scrollback, or local workflow-run directories.
...
Approved `merge-cleanup` and `tracker-refresh` tail steps stay human-required and surface as operator-decision actions instead of being started by the supervisor poller.

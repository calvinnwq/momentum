...
Provider-specific model aliases are normalized during the same pure route pass when enough context is present, so a Claude step configured as `{"harness":"claude","model":"sonnet"}` previews, persists, and dispatches the command-ready `claude-sonnet-4-6` model string instead of the bare alias.

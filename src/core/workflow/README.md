...
The resulting plan is a pure projection of the version-pinned built-in definition plus inputs, including the compatibility `route.implementationEngine` label, each step's executor-owned portable config, and its effective agent config, so a later `start-coding` from the same inputs persists a matching run.
...
`route/coding.ts` is the pure keystone for native per-step coding route/config overrides: it validates, normalizes, reads, and shapes operator `harness`/`model`/`effort` selections per configurable coding step (`implementation`, `postflight`, `validate`, `merge-cleanup`) under a byte-stable `route.steps` compatibility namespace; canonical run state lives in `workflow_steps.agent_config_json`, alongside the historical implementation-label and profile values in the canonical-only `workflow_run_coding_compatibility` destination, which the compatibility projection no longer emits.

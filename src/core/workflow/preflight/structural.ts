 ...
 * The CLI start and preview doors use it to fail closed before durable run materialization when the built-in definition, per-step agent config, run-start shape, or required inputs are structurally invalid.
 * The coding workflow wrapper config validator uses the same evidence shape for field-level setup failures, including canonical snake_case guidance, built-in process timeout bounds, result-file path checks, and no-mistakes runner-profile shape / env-allowlist checks.
 *

...
- `update-intent-generator.ts` - `source_satisfied` update-intent generation
  from goal completion plus verification evidence. The durable intent-type
  value keeps its recorded `source_satisfied` spelling; only active symbols
  and surfaces use tracker vocabulary.

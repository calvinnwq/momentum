...
Lists source items stored in the data directory. When `--adapter` is provided, filters to items from that adapter kind only. JSON output includes `ok`, `command`, `dataDir`, `adapter`, `count`, and an `items` array where each element has `id`, `adapterKind`, `externalId`, `externalKey`, `url`, `title`, `status`, `metadata`, `lastObservedAt`, `goalId`, `createdAt`, and `updatedAt`. Text output lists adapter kind, external key / id, title, and status for each item.

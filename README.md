...
- **reconciliation run** — a single invocation of a source adapter that records started/finished timestamps, adapter kind, input filters or other metadata, state, top-level error, and `items_seen` / `items_upserted` counts; richer created / updated / skipped counts and per-item error metadata are future M5 work.

...
The envelope shape and its enum vocabularies (`reason`, `disposition`, `phase`, `cleanup`, `recommendedAction`, `stuckRisk`, and `humanAction.code`) are frozen by `test/workflow-watch-contract.test.ts`, which fails if a required field disappears or an enum value drifts.
...
OpenClaw local auto-actions remain config-gated through `MOMENTUM_OPENCLAW_AUTO_ACTIONS`, execute only explicitly supported `auto_allowed` policies, append per-attempt audit JSONL beside the supervisor state before applying any local state change, append the final saved/failed state-persistence status for successful actions, bound repeated `release_monitor` state-save attempts to three saved records per digest without resetting that count for intervening failed persistence records, and fail closed to human-required when policy support, repeat bounds, or audit persistence are ambiguous.
...
Fail-closed auto-action escalations preserve sanitized human-review delivery text, suppress monitor-removal cleanup unless a disabled-monitor escalation audit was saved, and treat required final audit status write failure as a nonzero `openclaw_auto_action_audit_failed` refusal.

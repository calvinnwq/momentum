...
definition resolution, approval boundary, per-step agent config, wrapper/host-binding schema,
...
source so durable status/list/handoff/monitor/logs show it as Momentum-owned. It
captures the run's isolation inputs in durable state: repo, objective, issue
scope, approval boundary, and skill revision. Historical operator
runtime/profile and implementation-label compatibility values recorded by
earlier releases remain read-back/refusal evidence only; fresh runs record
none.
...
`workflow run preview-coding` is the read-only native plan-preview door: it shares the `start-coding` preconditions, built-in definition resolution, and configured executor module/schema preflight but writes no Momentum state, emitting a frozen plan (run id, repo, objective, issue scope, approval boundary, definition key/version, and every step with its executor identity, portable config, effective agent config, and on-start state) so an operator can inspect the proposed run before approval or execution.
The recorded definition key/version, matching versioned step executor, and frozen portable config in the owning workflow step are the active implementation authority; historical compatibility labels cannot choose a different executor or host binding.
...
Structural preflight is shared by the native coding start and preview doors before durable run writes: missing built-in definition versions, blank required repository paths, invalid approval boundaries, invalid issue-scope identifiers, and invalid per-step agent config fail closed with `preflightEvidence`.
The retired `--profile`, `--implementation-engine`, and `--steps-json` flags fail as usage errors before any durable write, with no alias or fallback; historical `workflow_run_coding_compatibility` labels (`gnhf`, `native-goal-loop`, `current-gnhf-cwfp`) keep read-back/refusal semantics only.
Implementation execution semantics are owned by [Daemon commands](docs/daemon.md#workflow-host-bindings).
The coding doors carry native per-step agent config: `workflow run start-coding` / `workflow run preview-coding` accept `--agent-config-json <json>` (replacing the retired `--steps-json` flag), a sparse object keyed by the configurable coding steps (`implementation`, `postflight`, `validate`, `merge-cleanup`) carrying `harness`/`model`/`effort` string fields.
...
Status detail exposes that effective per-step selection as step-level `agentConfig` for durable audit; the monitor path derives its compact envelope from the same typed detail loader, and native dispatch reads the same frozen step rows.
Retry and reattachment round materialization reuses the same frozen step-owned selection; no other durable selection representation exists.
...
implementation-engine values for their remaining fail-closed refusal semantics;
those historical values do not select the executor or its configuration.
...
Durable workflow state uses explicit destinations: per-step agent and executor config on `workflow_steps`, optional definition agent config on `step_definitions`, child ancestry in `workflow_run_lineage` (projected as `run.lineage`), historical non-imported engine/profile compatibility in `workflow_run_coding_compatibility` (projected under `run.compatibility.coding` only when labels exist), and imported mode/profile/risk/quota policy plus the imported source format and import timestamps in `workflow_run_import_metadata` (projected as `run.importMetadata`); run source provenance remains on `workflow_runs`.
The retired `workflow_runs.route_json` column and the route compatibility projector no longer exist: opening a pre-rebuild data directory migrates every legacy route value into its explicit destination, verifies equivalence and foreign keys, and transactionally rebuilds `workflow_runs` without the column, while a second open is a no-op and malformed or unmappable legacy state fails closed without changing the original database.
Import metadata and implementation authority are canonical-only: import persistence and operator read-back use `workflow_run_import_metadata` and `workflow_run_coding_compatibility` as their canonical owners, native dispatch validates only the canonical compatibility refusal state, imported metadata (including the imported profile) is historical audit state that never influences executor or host selection, and the recorded definition key/version, matching versioned step-definition executor, and step-owned frozen config alone select what runs.
Compatibility values keep only their existing fail-closed refusal semantics before native dispatch - the required marker validation and the explicit `current-gnhf-cwfp` refusal - so a compatibility row can refuse a dispatch but can never select a different executor, rewrite definition identity, or alter the frozen portable config.
Subworkflow facts are canonical-only: the owning step's `workflow_steps.executor_config_json` `child` object is the only active source of child intent, the run's `workflow_run_lineage` row is the only active source of parent/depth/ancestry, and a subworkflow child run is started with an explicit start-persistence lineage input (the retired `route.subworkflow` start namespace is refused).
Upgrades pre-scan the complete legacy inventory before mutation, then migrate destination schema, canonical values, equivalence validation, route clearing, and foreign-key validation in one fail-closed `BEGIN IMMEDIATE` transaction before the table rebuild removes the emptied column.

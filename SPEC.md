...
unique issue-scope/source evidence to seed the expected pending `status_update`
intent deterministically. The resulting intent must carry a valid payload with
exactly one `state` / `stateId` and the stable idempotency marker.

...
The envelope shape, enum vocabularies (`reason`, `disposition`, `phase`, `cleanup`, `recommendedAction`, `stuckRisk`, `nextAction.code`, and `humanAction.code`), and common GUI scenarios are frozen by `test/fixtures/workflow-gui-contract.json` and `test/workflow-watch-contract.test.ts`, which fail if a required field disappears, an enum value drifts, or a scenario loses its expected human action, null action, failed-step next action, or stuck-risk inspection command.

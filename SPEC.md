...
The watch tick re-reads durable state after that bounded dispatch, persists the same digest baseline as monitor advance, and still never resolves gates, approvals, or recovery decisions by itself.
...
Its top-level field set is fixed, `emit` is the machine-polling signal that suppresses a repeated identical tick while `reason`/`disposition` stay stable for the human-facing update, and it adds the watch-derived `recommendedAction`, `nextPollSeconds`, `quietForSeconds`, `stuckRisk`, and `humanAction` fields over the shared monitor projection so a consumer can branch and render a concise human update without a follow-up `workflow status` read.
The envelope shape, its enum vocabularies (`reason`, `disposition`, `phase`, `cleanup`, `recommendedAction`, `stuckRisk`, and `humanAction.code`), and the seven supervisor scenarios are frozen by `test/workflow-watch-contract.test.ts`, which fails if a required field disappears or an enum value drifts.

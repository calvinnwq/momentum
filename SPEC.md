...
The read-only `route.steps` projection exposes the effective per-step selection for durable audit by status/handoff/monitor/logs and for dispatcher-created executor-round selection; definition-level agent config and run-specific overrides are merged in canonical step state, and the projection stays distinct from the daemon's `MOMENTUM_LIVE_WRAPPER_PROFILE` execution profile.

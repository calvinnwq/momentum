                ...
                nodes: [
                  {
                    id: "issue-with-body",
                    identifier: "NGX-290",
                    title: "Goal/tracker linkage",
                    description:
                      "Use this Linear issue body in planning context.",
...
describe("buildLinearHttpReconciliationClient — request shape and read-only contract", () => {
  ...
  it("trims surrounding whitespace from the credential before sending it", async () => {
    ...
    await client.fetchPage({ cursor: null, filters: {} });
    expect(requireFirstCall(calls).init.headers["Authorization"]).toBe(
      "lin_padded_key",
    );
  });
});
...
describe("buildLinearHttpReconciliationClient — auth and transport failure taxonomy", () => {
  it("returns tracker_auth_unavailable without calling fetch when the api key is empty", async () => {
    const { fetch, calls } = recordingFetch(() => {
      throw new Error(
        "fetch must not be called when the credential is missing",
      );
    });
  ...
  it("returns tracker_config_invalid when no fetch implementation is available", async () => {
  ...
  it("maps HTTP 401 to tracker_auth_unavailable", async () => {
  ...
  it("maps HTTP 403 to tracker_auth_unavailable", async () => {
  ...
  it("maps a non-auth non-OK status to tracker_adapter_threw", async () => {
    ...
    const result = await client.fetchPage({ cursor: null, filters: {} });
    expect(result).toMatchObject({ ok: false, code: "tracker_adapter_threw" });
    if (result.ok) return;
    expect(result.error).toContain("500");
  });

  it("maps a thrown transport error to tracker_adapter_threw", async () => {
  ...
  it("maps a failed response body read to tracker_adapter_threw", async () => {
    const { fetch } = recordingFetch(() => ({
      ok: true,
      status: 200,
  ...
  it("maps GraphQL auth errors flagged by extension code to tracker_auth_unavailable", async () => {
    const { fetch } = recordingFetch(() =>
      jsonOk({
        errors: [
          { message: "denied", extensions: { code: "AUTHENTICATION_ERROR" } },
        ],
      }),
    );
    const client = buildLinearHttpReconciliationClient({
  ...
  it("maps GraphQL auth errors detected by message to tracker_auth_unavailable", async () => {
    const { fetch } = recordingFetch(() =>
  ...
  it("maps non-auth GraphQL errors to tracker_adapter_threw with the message", async () => {
    const { fetch } = recordingFetch(() =>
...
describe("buildLinearHttpReconciliationClient — pagination cursor handling", () => {
  async function fetchPageInfo(
    ...
    const { fetch } = recordingFetch(() =>
      jsonOk(issuesPageBody({ nodes: [], pageInfo })),
    );
    const client = buildLinearHttpReconciliationClient({
      apiKey: "lin_api_key",
      fetch,
  ...
  it("returns the end cursor when another page is available", async () => {
    const result = await fetchPageInfo({
      hasNextPage: true,
      endCursor: "cursor-2",
    });
    expect(result).toMatchObject({
  ...
  it("forwards the supplied cursor as the GraphQL after variable", async () => {
    ...
    expect(parseRequestBody(requireFirstCall(calls)).variables["after"]).toBe(
      "page-2-cursor",
    );
  });
...
describe("buildLinearHttpReconciliationClient — no real api.linear.app calls", () => {
  it("defaults to the documented real Linear GraphQL endpoint constant", () => {
    expect(DEFAULT_LINEAR_GRAPHQL_ENDPOINT).toBe(

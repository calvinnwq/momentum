...
import { TRACKER_RECONCILIATION_RUN_STATES } from "../src/core/tracker/reconciliation-runs.js";
...
describe("adapter test coverage contract", () => {
  ...
  it("keeps compact adapter boundary anchors in SPEC.md", () => {
    expectSpecSection(spec, "Tracker And Adapter Boundaries");
  ...
  it("keeps source reconciliation states explicit", () => {
    expect([...TRACKER_RECONCILIATION_RUN_STATES]).toEqual([
      "running",
      "succeeded",
      "failed",
    ]);
  ...
  it("continues to prove adapter composition with executable tests", () => {
    for (const rel of [
      ...
      "test/linear-tracker-adapter.test.ts",
      "test/tracker-reconciliation.test.ts",

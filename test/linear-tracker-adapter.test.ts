...
import {
  buildLinearTrackerAdapter,
  normalizeLinearIssue,
  type LinearTrackerAdapterClient,
} from "../src/adapters/linear-tracker-adapter.js";

const ISSUE_ONE_RAW = {
  id: "d901abc0-b872-489c-a07c-aef4dd777c51",
  identifier: "NGX-289",
  title: "M5-02 Linear source adapter read and reconciliation",
  url: "https://linear.app/ngxcalvin/issue/NGX-289/m5-02-linear-tracker-adapter-read-and-reconciliation",
...
const ISSUE_TWO_RAW = {
  id: "issue-2-uuid",
  identifier: "NGX-290",
  title: "M5-03 Goal/tracker linkage and planning context",
...
function linearClient(
  client: LinearTrackerAdapterClient,
): TrackerAdapterClient {
  return { linear: client };
}

describe("normalizeLinearIssue", () => {
  it("maps Linear issue fields onto a normalized TrackerAdapterItem", () => {
    ...
    expect(result.item.observedAt).toBe(Date.parse(ISSUE_ONE_RAW.updatedAt));
  });
...
describe("linear adapter list/get via dispatchTrackerAdapter*", () => {
  it("lists all normalized Linear issues injected through the client", () => {
    const out = dispatchTrackerAdapterList("linear", {
      client: linearClient({
  ...
  it("returns tracker_item_invalid if any injected raw issue cannot be normalized", () => {
    const out = dispatchTrackerAdapterList("linear", {
      client: linearClient({
        issues: [
          ISSUE_ONE_RAW,
  ...
  it("gets a single Linear issue by external id (Linear UUID)", () => {
    const out = dispatchTrackerAdapterGet("linear", ISSUE_ONE_RAW.id, {
      client: linearClient({
        issues: [ISSUE_ONE_RAW, ISSUE_TWO_RAW],
      }),
    });
  ...
  it("gets a single Linear issue by external key (identifier)", () => {
    const out = dispatchTrackerAdapterGet("linear", "NGX-290", {
      client: linearClient({
        issues: [ISSUE_ONE_RAW, ISSUE_TWO_RAW],
      }),
    });
  ...
  it("returns tracker_item_not_found when no injected issue matches", () => {
    const out = dispatchTrackerAdapterGet("linear", "missing", {
      client: linearClient({
...
describe("dispatchTrackerAdapterNormalize for linear", () => {
  it("normalizes a raw Linear payload through the dispatch boundary", () => {
    const out = dispatchTrackerAdapterNormalize("linear", ISSUE_ONE_RAW);
    expect(out.ok).toBe(true);
    if (!out.ok) return;
    expect(out.item.externalKey).toBe("NGX-289");
  });
...
describe("buildLinearTrackerAdapter", () => {
  it("exposes the linear kind label", () => {
    const adapter = buildLinearTrackerAdapter();

...
describe("Milestone 5 evidence + intent + project status smoke (NGX-294)", () => {
  ...
  it("ingests workflow fixtures and surfaces them through evidence list and doctor", () => {
    ...
    const createdTypes = (
      ingestPayload["created"] as Array<Record<string, unknown>>
    )
    ...
    expect(reIngest.code, `re-ingest stderr: ${reIngest.stderr}`).toBe(0);
    ...
    const listedTypes = (
      listPayload["records"] as Array<Record<string, unknown>>
    )
      .map((record) => record["type"])
      .sort();
    ...
    for (const record of records) {
      ...
      expect(
        (record["ingestKey"] as string).startsWith("agent-workflow:"),
      ).toBe(true);
    }
    ...
    expect(
      filtered.code,
      `filtered evidence list stderr: ${filtered.stderr}`,
    ).toBe(0);
    ...
    expect(
      (filteredPayload["records"] as Array<Record<string, unknown>>)[0]?.[
        "type"
      ],
    ).toBe("plan_created");
    ...
    const evidencePayload = doctorPayload["evidence"] as Record<
      string,
      unknown
    >;
  ...
  it("reports a deterministic project status rollup when no source items exist", () => {
    ...
    expect(result.code, `project status stderr: ${result.stderr}`).toBe(0);
    const payload = JSON.parse(result.stdout) as Record<string, unknown>;
    expect(payload).toMatchObject({
      ok: true,
  ...
  it("reconciles fixture Linear issues against a mock endpoint and surfaces them through source list and source get", async () => {
    ...
    try {
      ...
      expect((metadata["milestone"] as Record<string, unknown>)?.["name"]).toBe(
        "Milestone 5: Source Adapters And Evidence Sync",
      );

      const doctor = runCliBinary(["doctor", "--data-dir", dataDir, "--json"]);
      expect(doctor.code, `doctor stderr: ${doctor.stderr}`).toBe(0);
      const doctorPayload = JSON.parse(doctor.stdout) as Record<
        string,
        unknown
      >;
  ...
  it("links a reconciled SourceItem to a goal and surfaces the link through source get", async () => {
    ...
    try {
      ...
      const relinkPayload = JSON.parse(relink.stdout) as Record<
        string,
        unknown
      >;
  ...
  it("generates a source_satisfied update intent through source link for a completed goal and refuses --external-apply", async () => {
    ...
    try {
      ...
      // Ingest a workflow evidence fixture with `validate complete` so
      ...
      >;
      expect(
        ingestCreated.some((record) => record["type"] === "validate_complete"),
        `expected validate_complete in created evidence: ${JSON.stringify(
          ingestCreated,
        )}`,
      ).toBe(true);
  ...
  it("computes a project rollup with mismatches and pending intents through the built CLI", async () => {
    ...
    try {
      ...
      // Ingest workflow evidence with a validate complete entry so the

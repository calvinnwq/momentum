...
describe(
  ...
  { timeout: 60_000 },
  () => {
    it("rejects recovered no-mistakes success that contradicts the wrapper outcome", async () => {
      const repoPath = initRepo();
      const branch = runGit(repoPath, ["branch", "--show-current"]);
      const headSha = runGit(repoPath, ["rev-parse", "HEAD"]);
      const root = path.join(repoPath, ".agent-workflows", "outcome-mismatch");
      ...
      const resultContent = JSON.stringify({
        success: true,
        summary: "result file claims success",
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          attemptId: "outcome-mismatch::no-mistakes::dispatch",
          attempt: 1,
          phase: "completed",
          branch,
          headSha,
          ...
          executorLogPath,
          externalIdentity: {
            externalRunId: "nm-run-outcome-mismatch",
            branch,
            headSha,
          },
          resultDigest: `sha256:${crypto.createHash("sha256").update(resultContent).digest("hex")}`,
          worktreeTree,
          dispatchOutcome: {
            ok: false,
            code: "command_failed",
            error: "wrapper exited non-zero",
          },
        }),
      );
      const adapter = createProfileBackedDelegateToolAdapter({
        ...
        run: () => {
          throw new Error("recovery must not relaunch");
        },
      ...
      ).rejects.toThrow(/wrapper outcome is not safely recoverable/);
    ...
    it("rejects an initial no-mistakes result that contradicts its dispatch outcome", async () => {
      ...
      const adapter = createProfileBackedDelegateToolAdapter({
        tool: "no-mistakes",
        attemptId: "initial-outcome-mismatch::no-mistakes::dispatch",
        attempt: 1,
        branch,
        headSha,
        statePath,
    ...
    it("accepts correlated failed no-mistakes finalization as prepared recovery evidence", () => {
      ...
      const branch = runGit(repoPath, ["branch", "--show-current"]);
      const headSha = runGit(repoPath, ["rev-parse", "HEAD"]);
      const root = path.join(repoPath, ".agent-workflows", "failed-preflight");
      fs.mkdirSync(root, { recursive: true });
      const handoffReceiptPath = path.join(root, "delegate-handoff.json");
      const statePath = path.join(root, "delegate-external-state.json");
      const resultJsonPath = path.join(root, "result.json");
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          attemptId: "failed-preflight::no-mistakes::dispatch",
          attempt: 1,
          phase: "failed",
          branch,
          headSha,
          ...
          executorLogPath,
          externalIdentity: {
            externalRunId: "nm-run-failed-preflight",
            branch,
            headSha,
          },
          resultDigest: `sha256:${crypto.createHash("sha256").update(resultContent).digest("hex")}`,
          ...
          failureSummary: "repository ownership was lost before commit",
        }),
      );
    ...
    it("refuses mutable generic state when its authoritative receipt is invalid", () => {
      const repoPath = initRepo();
      const branch = runGit(repoPath, ["branch", "--show-current"]);
      const headSha = runGit(repoPath, ["rev-parse", "HEAD"]);
      const root = path.join(repoPath, ".agent-workflows", "invalid-receipt");
      fs.mkdirSync(root, { recursive: true });
      const receiptPath = path.join(root, "delegate-handoff.json");
      const statePath = path.join(root, "delegate-external-state.json");
      const identity = {
        externalRunId: "generic-run-invalid-receipt",
        branch,
        headSha,
      };
      fs.writeFileSync(
        receiptPath,
        JSON.stringify({ schemaVersion: 1, tool: "gnhf", phase: "completed" }),
      );
      fs.writeFileSync(
        statePath,
        JSON.stringify({
      ...
      expect(
        adapter.readExternalState!({
          ...
          handoff: {
            ...
            summary: "generic handoff",
            artifactPaths: [statePath, receiptPath],
          },
          signal: new AbortController().signal,
        }),
      ).toMatchObject({ ok: false, error: expect.stringMatching(/receipt/) });
    });
    ...
    it("authorizes only the selected reset during generic handoff", async () => {
      ...
      const adapter = createProfileBackedDelegateToolAdapter({
        tool: "gnhf",
        attemptId: "reset-handoff::implementation::dispatch",
        attempt: 1,
        branch,
        headSha,
        statePath,
    ...
    it("refuses an initial generic result that contradicts its dispatch outcome", async () => {
      ...
      const adapter = createProfileBackedDelegateToolAdapter({
        tool: "gnhf",
        attemptId: "result-race::implementation::dispatch",
        attempt: 1,
        branch,
        headSha,
        statePath,
    ...
    it("preserves staged-only delegate changes instead of overwriting the index", async () => {
      const repoPath = initRepo();
      const branch = runGit(repoPath, ["branch", "--show-current"]);
      const headSha = runGit(repoPath, ["rev-parse", "HEAD"]);
      const root = path.join(repoPath, ".agent-workflows", "index-only");
      fs.mkdirSync(root, { recursive: true });
      const handoffReceiptPath = path.join(root, "delegate-handoff.json");
      const statePath = path.join(root, "delegate-external-state.json");
      const resultJsonPath = path.join(root, "result.json");
      ...
      const adapter = createProfileBackedDelegateToolAdapter({
        tool: "gnhf",
        attemptId: "index-only::implementation::dispatch",
        attempt: 1,
        branch,
        headSha,
        statePath,
        ...
        run: () => {
          fs.writeFileSync(path.join(repoPath, "README.md"), "staged only\n");
          ...
          fs.writeFileSync(
            ...
            JSON.stringify({
              success: true,
              summary: "staged-only delegate result",
              key_changes_made: [],
              key_learnings: [],
              remaining_work: [],
              goal_complete: false,
      ...
      ).rejects.toThrow(/index differs from the captured worktree/);
      expect(
        runGit(repoPath, ["diff", "--cached", "--", "README.md"]),
      ).toContain("staged only");
      expect(fs.readFileSync(path.join(repoPath, "README.md"), "utf8")).toBe(
        "fixture\n",
      );
    });

    it.each(["completed", "resetting"] as const)(
      "authorizes only the selected reset from a %s generic receipt",
      async (receiptPhase) => {
        const repoPath = initRepo();
        const branch = runGit(repoPath, ["branch", "--show-current"]);
        const headSha = runGit(repoPath, ["rev-parse", "HEAD"]);
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            schemaVersion: 1,
            tool: "gnhf",
            attemptId: "reset-recovery::implementation::dispatch",
            attempt: 1,
            phase: receiptPhase,
            baseHead: headSha,
            branch,
    ...
    it("refuses a prepared generic commit after repo ownership is lost", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          tool: "gnhf",
          attemptId: "commit-ownership::implementation::dispatch",
          attempt: 1,
          phase: "finalizing",
          baseHead: headSha,
          branch,
    ...
    it("rechecks repo ownership immediately before a prepared no-mistakes commit", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          attemptId: "no-mistakes-ownership::implementation::dispatch",
          attempt: 1,
          phase: "finalizing",
          branch,
          headSha,
...
`,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          tool: "gnhf",
          attemptId: "recovery-digest::implementation::dispatch",
          attempt: 1,
          phase: "completed",
          baseHead: headSha,
          branch,
...
`,
      ...
      const adapter = createProfileBackedDelegateToolAdapter({
        tool: "no-mistakes",
        attemptId: "changed-verification::no-mistakes::dispatch",
        attempt: 1,
        branch,
        headSha,
        statePath,
    ...
    it("reloads durable terminal proof for persisted no-mistakes polling", async () => {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          attemptId: "persisted-proof::no-mistakes::dispatch",
          attempt: 1,
          phase: "launched",
          branch,
          headSha,
...
`,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          schemaVersion: 1,
          attemptId: "retry-digest::no-mistakes::dispatch",
          attempt: 1,
          phase: "failed",
          branch,
          headSha,
...
`,
    ...
    it("recovers a committed handoff from its finalization receipt", async () => {
      ...
      expect(receipt).toMatchObject({
        phase: "finalized",
        attemptId: `${runId}::${stepId}::dispatch`,
      });
    ...
    it("recovers failed no-mistakes finalization through production dirty-worktree preflight", async () => {
      const dataDir = tempDir();
      const repoPath = initRepo();
      const runId = "no-mistakes-failed-dirty-recovery";
      const stepId = "no-mistakes";

...
describe("loadCodingWorkflowWrapperConfig", () => {
  ...
  it("fails closed before spawning no-mistakes when the selected route agent differs from runner profile", () => {
    const dir = makeTempDir();
    const repo = path.join(dir, "repo");
    const iteration = path.join(dir, "run");
    const resultPath = path.join(iteration, "result.json");
    ...
    expect(outcome.exitCode).toBe(1);
    expect(outcome.success).toBe(false);
    expect(outcome.summary).toContain("selected agent claude");
    expect(fs.existsSync(sentinelPath)).toBe(false);
    expect(fs.existsSync(resultPath)).toBe(false);
  });

  it("fails closed when no-mistakes config contains malformed scalar syntax", () => {
    const dir = makeTempDir();
    const repo = path.join(dir, "repo");
    const iteration = path.join(dir, "run");
    const resultPath = path.join(iteration, "result.json");
    ...
    expect(outcome.exitCode).toBe(1);
    expect(outcome.success).toBe(false);
    expect(outcome.summary).toContain("malformed scalar");
    expect(fs.existsSync(sentinelPath)).toBe(false);
    expect(fs.existsSync(resultPath)).toBe(false);
  });
  ...
  it("fails closed when no-mistakes config has quoted duplicate top-level agent keys", () => {
    const dir = makeTempDir();
    const repo = path.join(dir, "repo");
    const iteration = path.join(dir, "run");
    const resultPath = path.join(iteration, "result.json");
  ...
  it("fails closed when no-mistakes config has quoted duplicate agent path override keys", () => {
    const dir = makeTempDir();
    const repo = path.join(dir, "repo");
    const iteration = path.join(dir, "run");
    const resultPath = path.join(iteration, "result.json");
  ...
  it("fails closed when no-mistakes config omits YAML separators", () => {
    const dir = makeTempDir();
    const repo = path.join(dir, "repo");
    const iteration = path.join(dir, "run");
    const resultPath = path.join(iteration, "result.json");
  ...
  it("fails closed when no-mistakes agent path override has list entries", () => {
    const dir = makeTempDir();
    const repo = path.join(dir, "repo");
    const iteration = path.join(dir, "run");
    const resultPath = path.join(iteration, "result.json");
    ...
    expect(outcome.exitCode).toBe(1);
    expect(outcome.success).toBe(false);
    expect(outcome.summary).toContain("agent_path_override");
    expect(fs.existsSync(sentinelPath)).toBe(false);
    expect(fs.existsSync(resultPath)).toBe(false);
  });

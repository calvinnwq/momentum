...
function workflowRunMonitorJson(
  dataDir: string,
  runId: string
): Record<string, unknown> {
  const result = runCliBinary([
...
describe("Milestone 8 operator-control end-to-end smoke (NGX-330)", () => {
  it(
    "composes list / approve / monitor / typed evidence linkage through the built CLI",
    () => {
      ...
      expect(
        (
          ...
          (entry) =>
            entry.run.runId === runId && entry.run.state === "succeeded"
        )
      ).toBe(true);
      ...
      const detail = workflowStatusJson(dataDir, [runId]);
      const detailEvidence = detail["evidence"] as Array<
        Record<string, unknown>
      >;
  ...
  it(
    "recovers a ghost-active run: flag, monitor recover, clear-recovery refusal, update-step resolution, then a clean clear",
    () => {
...
describe("Milestone 10 production workflow-lane dispatch smoke (NGX-367)", () => {
  it(
    "drives workflow run start -> approve -> daemon start --max-* -> durable executor rows -> status/handoff/monitor through the built CLI",
    () => {
      ...
      expect(
        approve.code,
        `workflow run approve stderr: ${approve.stderr}`
      ).toBe(0);
      ...
      expect(
        statusSteps.find((step) => step.stepId === "preflight")?.state
      ).toBe("running");

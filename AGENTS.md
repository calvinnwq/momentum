...
Milestone 10 (Workflow-First Runtime) is the active implementation milestone after M10-00 promoted the workflow-first planning contracts into an executable sequence. M10-01 adds the pure `WorkflowDefinition` / `StepDefinition` shape, deterministic validation, durable `workflow_definitions` / `step_definitions` persistence, and the built-in coding workflow definition; M10-02 adds first-class workflow run start from persisted or built-in definitions; M10-03 adds executor invocation / round persistence; M10-04 adds the opt-in daemon workflow scheduler lane; M10-05 adds the `goal-loop` executor adapter (`runGoalLoopStep`) that drives bounded autonomous rounds below a step run, reusing the M9 verify / commit / reset finalization and the M10-03 round persistence; M10-06 adds the `one-shot` and `script` executor adapters; M10-07 adds the no-mistakes executor mirror; M10-08 adds durable workflow / step / executor gates (the `workflow_gates` table plus the pure `evaluateGateDecision` brain) and the `workflow run decide` operator + delegated-policy decision CLI, with gate visibility surfaced in `workflow status` / `workflow handoff` / `workflow run monitor`. Closeout remains later M10 work. The `doctor --json` marker stays on the M8 closeout string above until M10 closeout (NGX-353). See [`internal/milestones/m10-workflow-first-runtime.md`](internal/milestones/m10-workflow-first-runtime.md), [`internal/contracts/workflow-first-runtime.md`](internal/contracts/workflow-first-runtime.md), [`internal/contracts/executor-loop.md`](internal/contracts/executor-loop.md), and [`internal/contracts/workflow-first-gap-matrix.md`](internal/contracts/workflow-first-gap-matrix.md).

...
- Initial M4 runner family: `fake` (existing in-process baseline), `trusted-shell` (recognized placeholder until NGX-282 lands shell execution), and one live ACP/acpx-style runtime as the smoke path if available locally (otherwise opt-in env-gated).

...
Post-M11 repo architecture planning is recorded in [`internal/contracts/repo-architecture-standard.md`](internal/contracts/repo-architecture-standard.md). It defines the target `src/commands`, `src/renderers`, `src/adapters`, `src/config`, `src/shared`, and `src/core/<domain>` taxonomy for ARCH-02..ARCH-08. ARCH-02 enforces the root `src/*.ts` allowlist, transitional exception metadata, placeholder-free pending homes, and renderer/core import guardrails; ARCH-03 regrouped the workflow runtime under [`src/core/workflow/`](src/core/workflow/), ARCH-04 the executor runtime under [`src/core/executors/`](src/core/executors/), and ARCH-05 the remaining pseudo-domains under `src/core/<domain>/` plus [`src/config/`](src/config/). None of these slices authorize runtime changes, public CLI behavior changes, or compatibility-path deletion by itself.

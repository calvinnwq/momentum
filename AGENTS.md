...
- Initial M4 runner family: `fake` (existing in-process baseline), `trusted-shell` (operator-trusted shell runner with no sandbox; landed in NGX-282), and one live ACP/acpx-style runtime as the smoke path if available locally (otherwise opt-in env-gated).

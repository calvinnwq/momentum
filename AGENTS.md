...
- Milestone 3: Operational Safety is in active implementation. NGX-272 (M3-01 orchestrator state model and daemon CLI contract), NGX-273 (M3-02 managed daemon loop for queued jobs), NGX-274 (M3-03 graceful daemon stop visibility), and NGX-275 (M3-04 immediate daemon stop-now cancellation) have shipped; the remaining slices (cooperative mid-job shutdown semantics, automatic stale-lease recovery, manual recovery artifacts, and smoke/docs closeout) are still planning targets and must preserve the durable Goal/Iteration/Job/Handoff model.

...
Milestone 10 (Workflow-First Runtime) is the active implementation milestone. It is replacing the old goal-first top level with persisted workflow definitions/runs, step definitions/runs, executor invocations/rounds, daemon scheduling, executor adapters, durable workflow / step / executor gates plus the `workflow run decide` operator path, and production workflow-lane dispatcher wiring for bounded managed `daemon start`. Keep detailed slice history in internal docs, not here. Closeout remains later M10 work, and the `doctor --json` marker stays on the M8 closeout string above until M10 closeout (NGX-353). See [`internal/milestones/m10-workflow-first-runtime.md`](internal/milestones/m10-workflow-first-runtime.md), [`internal/contracts/workflow-first-runtime.md`](internal/contracts/workflow-first-runtime.md), [`internal/contracts/executor-loop.md`](internal/contracts/executor-loop.md), and [`internal/contracts/workflow-first-gap-matrix.md`](internal/contracts/workflow-first-gap-matrix.md).

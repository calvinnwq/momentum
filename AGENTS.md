...
- Milestone 4: Real Runner Profiles is the **active milestone** and is **not yet complete**. NGX-279 (M4-00 M4 contract, roadmap, and docs setup) is done. NGX-280 (M4-01 runner profile model and resolver) is done — `src/runner-profile.ts` ships `BUILTIN_RUNNER_KINDS`, `DEFAULT_RUNNER_KIND`, `parseRunnerProfile`, `resolveRunnerProfile` (cli_override > goal_frontmatter > builtin_default precedence), and `safeRunnerProfileSummary`; `goal-init.ts` validates runners at init time with stable error codes (`parse_error` / `unsupported_runner` / `malformed_profile` / `init_failed` replacing the generic `init_error`); `runnerProfile` and `runnerProfileSource` are surfaced on `goal start` JSON, `runnerProfile` is surfaced on `status` and `handoff`, and `doctor` exposes the built-in runner registry; both profiles (`fake` and `trusted-shell`) have `executes: false` in M4-01; the execution-layer check (`foreground-iteration.ts` rejecting non-`fake` runners at execution time) is intentional and will be replaced by the `RunnerAdapter` boundary in NGX-281. The planned issue order is NGX-281 (M4-02 RunnerAdapter boundary and fake-runner migration) → NGX-282 (M4-03 trusted-shell runner profile) → NGX-283 (M4-04 ACP/acpx runtime smoke runner) → NGX-284 (M4-05 runtime `MOMENTUM.md` policy loader) → NGX-285 (M4-06 real-runner status/logs/recovery hardening) → NGX-286 (M4-07 M4 smoke, docs, and milestone closeout). The M4 closeout marker will be pinned by NGX-286; until then the `doctor` milestone string intentionally still reads as the M3 closeout marker. M4 must not break any M3 surfaces (`daemon start` / `daemon stop` / `daemon status` / `recovery clear` shapes; `daemon_runs` / `repo_locks` / `goals.needs_manual_recovery` schema; stale-lease detection and startup-recovery; manual-recovery artifact + flag; `status` / `handoff` / `doctor` daemon/recovery fields).
...
- Initial M4 runner family: `fake` (existing in-process baseline), `trusted-shell` (operator-trusted shell command), and one live ACP/acpx-style runtime as the smoke path if available locally (otherwise opt-in env-gated).

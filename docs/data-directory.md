...
- `workflow_runs` - durable workflow run rows keyed by `runId`, carrying `state`, identity columns (`goal_id`, `repo_path`, `objective`, `issue_scope_json`, compatibility `route_json`, `approval_boundary`, `skill_revision`, nullable `workflow_definition_key` / `workflow_definition_version` provenance for definition-started runs), the run `source` (`agent-workflow`, `workflow-definition`, or `momentum-native-coding`) plus optional `source_artifact_path`, the captured `plan_json` body, optional batch grouping, monitor advisory columns (`monitor_last_seen_state`, `monitor_terminal`, `monitor_step`, `monitor_last_seen_digest`, `monitor_last_emitted_digest`, `monitor_last_seen_at`, `monitor_last_emitted_at`), the per-run `needs_manual_recovery` flag, run-scoped manual-recovery fields (`manual_recovery_reason`, `manual_recovery_at`) used by live recovery and scheduler-lane stale workflow-lease recovery when `recovery.md` rendering is best-effort or absent, and lifecycle timestamps.
  ...
  Status, list, handoff, and logs read-back expose only the remaining compatibility `route.steps` projection from the explicit destinations below; their shared typed loaders, also used to derive monitor state, supply historical implementation/profile and imported metadata fields, while native dispatch uses canonical definition and step state rather than this projection. Canonical-only subworkflow child intent and lineage are deliberately absent from the projection.
  ...
  Imported mode, legacy profile, risk, quota policy, and the import marker's `created_at` / `updated_at` timestamps are stored in `workflow_run_import_metadata`, with the quota policy kept in the single-purpose `quota_policy_json` column.
...
- `workflow_run_coding_compatibility` - one optional row per non-imported workflow run carrying historical implementation-engine and selected-profile compatibility values for read-back/refusal only.
- `workflow_run_import_metadata` - one optional row per imported run carrying historical mode, legacy profile, risk, named quota policy JSON, and import marker timestamps; the run's source provenance remains on `workflow_runs.source` and `workflow_runs.source_artifact_path`.

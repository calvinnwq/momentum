...
- `workflow_leases` — durable monitor / managed-step / dispatch leases keyed by `(run_id, lease_kind)`; stores holder, acquired / expires / heartbeat / released timestamps (a non-null `released_at` marks the lease as cleanly released), and a `stale_policy` of `auto-release` or `manual-recovery-required`. Stale dispatch leases with terminal dispatcher evidence are reconciled before release; stale running dispatches without terminal evidence are parked for run-scoped manual recovery before release, except an unclassified running or capturing-result `delegate-supervisor` round with a durable `delegate_handoff_completed` checkpoint under an `auto-release` lease is released and made resumable so the same handoff can be reattached.
...
- `executor_rounds` - bounded executor-loop attempts or external-supervision turns keyed by `round_id` and referencing `executor_invocations`; stores attempt / round ordering, durable round state, execution metadata, executor recommendation, result summaries, key changes, key learnings, log paths, remaining work, verification status, verification command results, commit / recovery fields, and lifecycle timestamps.
  ...
  If the process stops after the handoff checkpoint but before classification, the unclassified running or capturing-result round remains resumable under the same invocation and does not authorize another handoff.

...
- `workflow_runs` - durable workflow run rows keyed by `runId`, carrying `state`, identity columns (`goal_id`, `repo_path`, `objective`, `issue_scope_json`, compatibility `route_json`, `approval_boundary`, `skill_revision`, nullable `workflow_definition_key` / `workflow_definition_version` provenance for definition-started runs), the run `source` (`agent-workflow`, `workflow-definition`, or `momentum-native-coding`) plus optional `source_artifact_path`, the captured `plan_json` body, optional batch grouping, monitor advisory columns (`monitor_last_seen_state`, `monitor_terminal`, `monitor_step`, `monitor_last_seen_digest`, `monitor_last_emitted_digest`, `monitor_last_seen_at`, `monitor_last_emitted_at`), the per-run `needs_manual_recovery` flag, run-scoped manual-recovery fields (`manual_recovery_reason`, `manual_recovery_at`) used by live recovery and scheduler-lane stale workflow-lease recovery when `recovery.md` rendering is best-effort or absent, and lifecycle timestamps.
  ...
  `route_json` remains as a compatibility column, and new writes keep it as the literal empty object.
  Current status, handoff, monitor, logs, watch, and dispatch output reconstructs the remaining compatibility `route` shape through one read-only compatibility projection over the explicit destinations below; canonical-only subworkflow child intent and lineage are deliberately absent from that projection.
  Upgrades pre-scan every existing `route_json` before mutation, then migrate recognized legacy values into those destinations, validate the projection and foreign keys, and clear each migrated `route_json` to `{}` in one fail-closed `BEGIN IMMEDIATE` transaction before any partial schema or canonical state can commit; malformed JSON, unknown keys, conflicting aliases, ambiguous or unmappable values, invalid lineage, or foreign-key failures refuse the upgrade.
  Refusals identify the affected run or database, stable JSON path, stable route-state code, detail, and repair guidance.
  Back up the database before upgrading if you need to return to a binary that expects legacy values in `route_json`.
  ...
  `selected_profile` is historical operator-selected profile compatibility recorded by generic and coding starts, never authority for machine-local host binding.
  New runs default the implementation engine to `gnhf`; legacy `native-goal-loop` rows remain readable, `current-gnhf-cwfp` remains an explicit unsupported compatibility selection, and unknown persisted values fail closed before dispatch.
  The execution semantics of those values are owned by [Daemon commands](daemon.md#workflow-live-wrapper-profile).
  Imported mode, profile, risk, and quota policy are stored in `workflow_run_import_metadata`, with the quota policy kept in the single-purpose `quota_policy_json` column.
  Subworkflow parent/step/depth/ancestry facts are stored in `workflow_run_lineage`.
  A present lineage row is validated against its parent subworkflow step and exact root-first ancestry closure before dispatch trusts it; an absent row represents a top-level run.
  Configured child intent is frozen in the owning subworkflow step's `workflow_steps.executor_config_json`, which active dispatch reads instead of compatibility route state.
  For fresh `momentum-native-coding` runs, definition-level agent config and run-specific per-step harness/model/effort selections are merged and normalized at start, then frozen in `workflow_steps.agent_config_json`; the effective values are projected into `route.steps`, and native dispatch reads the canonical row rather than the compatibility projection.
  Run-specific `--steps-json` selections with a known mapped harness (`claude`, `codex`, or `opencode`) store provider-specific model aliases in the command-ready form for that harness (for example Claude `sonnet` becomes `claude-sonnet-4-6`, Codex `openai/gpt-5.5` becomes `gpt-5.5`, and OpenCode `glm-5.2` becomes `opencode-go/glm-5.2`); aliases without matching harness context and unknown values remain free-form after trimming.
  Corrupt canonical agent config fails closed on read-back and native dispatch, without falling back to stale or conflicting `route.steps` data.
...
- `workflow_steps` - durable step rows keyed by `(run_id, step_id)` with `kind` (`preflight` / `implementation` / `postflight` / `validate` / `merge-cleanup` / `tracker-refresh`), `state`, ordering, frozen `agent_config_json` and `executor_config_json`, a `ledger_offset` pointer into the run's `ledger.jsonl`, stable `error_code` / `error_message` fields, and operator audit fields for manual transitions, external-tail recovery reconciliation, or interrupted no-mistakes success reconciliation from legacy checks-passed proof or structured deterministic evidence (`operator_reason`, `operator_actor`, `operator_evidence_pointer`, `operator_ledger_pointer`, `operator_transition_at`).
  Upgrades rename legacy kinds in place but preserve historical `step_id` values because event IDs, approval digests, and executor foreign keys depend on them.
- `workflow_approvals` - durable approval rows keyed by `(run_id, boundary)` from per-run `approval-<boundary>.json` artifacts, `workflow run approve`, or `workflow run start` / `workflow run start-coding` with `--approval-boundary`; stores actor, phrase, artifact path and digest, recorded / discharged timestamps.
  ...
  Approval rows are frozen evidence: each row retains its recorded boundary spelling and digest; synonym merge and gate-coverage behavior is documented in [Workflow commands](workflow-commands.md#workflow-run-approve).
...
- `step_definitions` - durable ordered step rows for a workflow definition keyed by `(definition_key, definition_version, step_key)` and referencing `workflow_definitions(key, version)`; each carries the step `kind`, the permanent executor identity that powers it, optional portable `config_json`, optional harness/model/effort `agent_config_json`, a `step_order`, a `required` flag, and lifecycle timestamps.
- `workflow_run_lineage` - one optional row per child run carrying the parent run and step foreign keys, a positive depth equal to the root-first ancestor-definition-key count, and the exact ordered ancestor-definition-key closure; duplicate or mismatched ancestry is invalid.
- `workflow_run_coding_compatibility` - one optional row per non-imported workflow run carrying the historical implementation engine and selected profile compatibility values.
- `workflow_run_import_metadata` - one optional row per imported run carrying mode, profile, risk, and the named quota policy JSON.
  New definitions use the canonical step kinds (`preflight` / `implementation` / `postflight` / `validate` / `merge-cleanup` / `tracker-refresh`) and built-in executors (`agent-loop` / `agent-once` / `delegate-supervisor` / `script` / `external-apply` / `subworkflow`).
...
- `executor_attempts` - one immutable executor attempt below a workflow step, keyed by `attempt_id` and referencing `(run_id, step_id)`; stores the permanent executor identity in `executor`, plus state, `attempt_number`, nullable `legacy_invocation_id` / `legacy_provenance` migration-provenance columns, and lifecycle timestamps.
  ...
  The supported read-only `workflow run events` and `workflow run watch --stream --jsonl` commands run this prerequisite migration chain before opening their read-only handles, so retained SDK-05 databases remain readable through those commands.
  ...
  `workflow run watch --once` does not create the first scaffold for an approved `merge-cleanup` or `tracker-refresh` tail step; it leaves that side-effecting dispatch on a human-required operator-decision path.

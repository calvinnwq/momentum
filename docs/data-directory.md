...
- `workflow_run_import_metadata` - one optional row per imported run carrying historical mode, legacy profile, risk, named quota policy JSON, source format, and import marker timestamps; the run's source provenance remains on `workflow_runs.source` and `workflow_runs.source_artifact_path`.

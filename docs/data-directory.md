...
- `workflow_approvals` — durable approval rows keyed by `(run_id, boundary)` from per-run `approval-<boundary>.json` artifacts, `workflow run approve`, or `workflow run start --approval-boundary`; stores actor, phrase, artifact path and digest, recorded / discharged timestamps. CLI-created rows without an artifact file store synthetic `workflow-run-approve://<run-id>/<boundary>` or `workflow-run-start://<run-id>/<boundary>` provenance plus a deterministic synthetic digest.

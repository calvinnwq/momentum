...
- `workflow_leases` - durable monitor / managed-step / dispatch leases keyed by `(run_id, lease_kind)`; stores holder, acquired / expires / heartbeat / released timestamps (a non-null `released_at` marks the lease as cleanly released), and a `stale_policy` of `auto-release` or `manual-recovery-required`.
  Stale dispatch leases with terminal dispatcher evidence are reconciled before release.
  Stale running dispatches without terminal evidence are parked for run-scoped manual recovery before release, except an unclassified running, capturing-result, or `mirroring_external_state` `delegate-supervisor` round with a durable handoff intent or completed handoff, or an unclassified running or capturing-result native `goal-loop`, `one-shot`, or `script` round with durable `mechanism_completed` evidence, under an `auto-release` lease.
  Those exceptions are released and made resumable so completed mechanism work is classified without being repeated.
  An unclassified delegate round with a mirrored gate checkpoint, gate-eligible decision, and `waiting_operator` observation is also resumed so it can finish classification and gate parking.
  A completed `continue` poll with a durable handoff in its history is released and resumed through the same path.
...
- `executor_invocations` - one configured executor session below a workflow step, keyed by `invocation_id` and referencing `(run_id, step_id)`; stores the permanent executor identity in the legacy `executor_family` column, plus state, attempt number, and lifecycle timestamps.
  Bounded `daemon start --max-*` and `workflow run watch --once` create the first invocation scaffold when they dispatch an eligible approved workflow step with a valid executor identity; the workflow dispatcher uses deterministic `<run-id>::<step-id>::dispatch` ids so re-entry finds the same scaffold instead of duplicating work.
  ...
  A profile-backed delegate-supervisor wrapper result instead becomes durable handoff and terminal-candidate evidence; the invocation and step remain non-terminal until a later external-state read receives a daemon-accepted terminal classification.
  ...
  Registered third-party executors use the same deterministic invocation id and daemon-owned state transitions; an absent or unloadable registration records a `manual_recovery_required` invocation with `runtime_unavailable` rather than manufacturing success.
...
- `executor_rounds` - bounded executor-loop attempts or external-supervision turns keyed by `round_id` and referencing `executor_invocations`; stores attempt / round ordering, durable round state, execution metadata, executor recommendation, result summaries, key changes, key learnings, log paths, remaining work, verification status, verification command results, commit / recovery fields, and lifecycle timestamps.
  Legacy dispatch creates a first pending round scaffold (`<invocation-id>::round-1`) before later executor work is driven.
  Registered SDK lifecycles, including native `goal-loop`, `one-shot`, and `script`, instead materialize their first round at index 0 through the durable envelope after host bindings resolve.
  ...
  If the process stops after a durable handoff intent or completed handoff exists but before classification, the unclassified running, capturing-result, or `mirroring_external_state` round remains resumable under the same invocation and does not authorize another handoff.
  Native `goal-loop`, `one-shot`, and `script` rounds with durable `mechanism_completed` evidence are likewise resumable for classification without repeating mechanism execution or commits.
  ...
  If the crash occurs before classification after a delegate has persisted a mirrored gate checkpoint, gate-eligible decision, and `waiting_operator` observation, the unclassified round is resumed under the same invocation so classification and gate parking can finish.
...
Correlated legacy delegate state or no-mistakes receipts at the run root migrate into this step-scoped layout during recovery only after invocation and branch checks plus current-head validation for finalized state; a legacy no-mistakes receipt must explicitly record successful handoff finalization.

...
- `workflow_runs` - durable workflow run rows keyed by `runId`, carrying `state`, identity columns (`goal_id`, `repo_path`, `objective`, `issue_scope_json`, `approval_boundary`, `skill_revision`, nullable `workflow_definition_key` / `workflow_definition_version` provenance for definition-started runs), the run `source` (`agent-workflow`, `workflow-definition`, or `momentum-native-coding`) plus optional `source_artifact_path`, the captured `plan_json` body, optional batch grouping, monitor advisory columns (`monitor_last_seen_state`, `monitor_terminal`, `monitor_step`, `monitor_last_seen_digest`, `monitor_last_emitted_digest`, `monitor_last_seen_at`, `monitor_last_emitted_at`), the per-run `needs_manual_recovery` flag, run-scoped manual-recovery fields (`manual_recovery_reason`, `manual_recovery_at`) used by live recovery and scheduler-lane stale workflow-lease recovery when `recovery.md` rendering is best-effort or absent, and lifecycle timestamps.
  ...
  Status, list, handoff, and logs read-back expose only the explicit canonical destinations below through typed loaders (also used to derive monitor state); native dispatch uses canonical definition and step state.
  Upgrading a pre-rebuild data directory pre-scans every legacy `route_json` value before mutation, migrates recognized legacy values into those destinations, validates equivalence and foreign keys in one fail-closed `BEGIN IMMEDIATE` transaction, and then transactionally rebuilds `workflow_runs` without the column; malformed JSON, unknown keys, conflicting aliases, ambiguous or unmappable values, invalid lineage, or foreign-key failures refuse the upgrade and leave the original database intact.
  Refusals identify the affected run or database, stable JSON path, stable route-state code, detail, and repair guidance.
  Back up the database before upgrading if you need to return to a binary that expects the legacy `route_json` column.
  ...
  Fresh runs record no historical labels (both columns stay `NULL`); migrated `gnhf` and legacy `native-goal-loop` rows remain readable, `current-gnhf-cwfp` remains an explicit unsupported compatibility selection, and unknown persisted values fail closed before dispatch.
  ...
  For fresh `momentum-native-coding` runs, definition-level agent config and run-specific per-step harness/model/effort selections are merged and normalized at start, then frozen in `workflow_steps.agent_config_json`; status detail exposes the effective values as step-level `agentConfig`, and native dispatch reads the same canonical row.
  Run-specific `--agent-config-json` selections with a known mapped harness (`claude`, `codex`, or `opencode`) store provider-specific model aliases in the command-ready form for that harness (for example Claude `sonnet` becomes `claude-sonnet-4-6`, Codex `openai/gpt-5.5` becomes `gpt-5.5`, and OpenCode `glm-5.2` becomes `opencode-go/glm-5.2`); aliases without matching harness context and unknown values remain free-form after trimming.
  Corrupt canonical agent config fails closed on read-back and native dispatch; no other durable selection representation exists to fall back to.
...
- `executor_rounds` - bounded executor turns or external-supervision reads keyed by `round_id` and referencing `executor_attempts` through `attempt_id` and `attempt_number` (formerly `invocation_id` and `attempt`); stores round ordering, durable round state, execution metadata, executor recommendation, result summaries, key changes, key learnings, log paths, remaining work, verification status, verification command results, commit / recovery fields, and lifecycle timestamps.
  ...
  Executor rounds freeze agent / model / effort metadata resolved from canonical step config when present, surfaced through the step-level `agentConfig` read-back, but carry no terminal evidence until the owning executor or daemon adapter records it.

...
- `workflow_runs` — durable coding-workflow run rows keyed by `runId`, carrying `state`, identity columns (`repo_path`, `objective`, `issue_scope_json`, `route_json`, `approval_boundary`, `skill_revision`), optional batch grouping, the per-run `needs_manual_recovery` flag, and lifecycle timestamps.

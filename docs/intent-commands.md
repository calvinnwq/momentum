...
When `--external-apply` is requested, JSON output also includes an `externalApply` block with the resolved adapter, target reference, audit id, reconcile status, and (on success) external refs (`issueId`, `issueKey`, `issueUrl`, `commentId`, `commentUrl`, `statusTransitioned`, `nextStateId`, `nextStateName`, `idempotencyMarker`, `alreadyApplied`). The `reconcile.status` field is `success` when the post-apply single-issue refresh confirmed the idempotency marker on Linear and updated the local SourceItem snapshot; `mismatch_persists` when the marker was not found in Linear comments; `stale_source` when Linear no longer recognizes the target; `refresh_failed` on transient refresh errors; `post_apply_reconcile_failed` on unexpected reconcile failures; `targeted_refresh_unsupported` for adapters that do not support targeted refresh; `pending` or `deferred` when reconcile was not attempted (early refusal or audit-incomplete paths); or `null` when no reconcile context applies.

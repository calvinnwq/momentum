...
`intent_apply_policy: external_apply_allowed`, a matching Linear tracker item, and
either one pending Linear `status_update` intent or enough unique issue-scope /
tracker evidence to seed the expected pending `status_update` intent with a `Done` payload
...
Missing issue scope, missing or ambiguous tracker evidence, duplicate intents, stale or mismatched applied evidence, missing credentials, policy denial, audit-incomplete, blocked, or other unsafe apply outcomes park the step for manual recovery rather than fabricating success.
...
a native `agent-loop`, `agent-once`, `script`, or `delegate-supervisor` round that was still
...
than entering the workflow as fake success. An
...
The retired `--profile` and `--implementation-engine` command-line selectors are removed and fail as usage errors before any durable write; historical `workflow_run_coding_compatibility` labels recorded by earlier releases (`gnhf`, legacy `native-goal-loop`, and `current-gnhf-cwfp`) stay readable through the `run.compatibility.coding` read-back and keep only their refusal semantics.
The recorded built-in definition key/version, matching versioned step executor, and frozen portable step config are authoritative for active implementation dispatch: current version 3 uses `delegate-supervisor`, while retained version 1 projects its recorded `goal-loop` executor according to the raw-identity compatibility rule in [SPEC.md](../SPEC.md), and retained version 2 keeps its recorded `delegate-supervisor` implementation and delegated tool configuration.
Historical labels do not select those executors, and both executor paths resolve their machine-local mechanism from the step-kind host binding.
...
The `--agent-config-json <json>` option on `workflow run start-coding` (replacing the retired `--steps-json` flag) merges and freezes per-step harness/model/effort selections in `workflow_steps.agent_config_json`, and `workflow run preview-coding --agent-config-json <json>` reports the same normalized selection without persisting a preview run.
...
Native dispatch, including retry and reattachment round materialization, validates compatibility values only for their refusal semantics, then resolves the active executor from the recorded definition key/version and the frozen agent selection from the canonical step row; no route compatibility projection exists to override either.
...
Missing auth, issue scope, target, tracker evidence, deterministic intent evidence, or missing valid payload fails closed with operator-actionable recovery evidence;

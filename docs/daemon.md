...
The `--implementation-engine <engine>` option on `workflow run start-coding` records the historical coding implementation label in `workflow_run_coding_compatibility.implementation_engine`; status, list, handoff, and logs read it back through the typed `run.implementationEngine` field, while the monitor detail loader reads the same canonical row before deriving its compact envelope, and when omitted, coding starts persist the `gnhf` label.
`workflow run preview-coding --implementation-engine <engine>` reports that same compatibility label without persisting a run.
Accepted values are `gnhf`, legacy `native-goal-loop`, and `current-gnhf-cwfp`.
The recorded built-in definition key/version, matching versioned step executor, and frozen portable step config remain authoritative for active implementation dispatch: current version 3 uses `delegate-supervisor`, while retained version 1 projects its recorded `goal-loop` executor according to the raw-identity compatibility rule in [SPEC.md](../SPEC.md), and retained version 2 keeps its recorded `delegate-supervisor` implementation and delegated tool configuration.
The historical `gnhf` and `native-goal-loop` labels do not select those executors, and both executor paths resolve their machine-local mechanism from the step-kind host binding.

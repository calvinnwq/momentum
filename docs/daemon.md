...
The version-pinned built-in definition remains authoritative for the implementation executor: current version 3 uses `delegate-supervisor`, including for the default `gnhf` route and the accepted legacy `native-goal-loop` label; retained version 1 projects its recorded `goal-loop` executor to the native `agent-loop` SDK lifecycle, while retained version 2 keeps its recorded `delegate-supervisor` implementation and delegated tool configuration.

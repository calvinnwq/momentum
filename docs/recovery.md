...
An expired active repo lock retained by an interrupted delegate intent is not cleared blindly; the next dispatcher may reclaim it only for the same deterministic invocation after all stored repository, run, job, holder, attempt, and deadline fields still match.

...
On a successful flagged clear, the same transaction resolves every open
`manual_recovery_required` gate for that run whose allowed actions include
`clear_recovery`, recording the command as the operator actor.
Other gate types and manual-recovery gates without that action remain open.
...
The next dispatcher may take it over only for the same deterministic invocation, either after lock expiry or after scheduler recovery proves and releases the matching stale dispatch owner, while all stored repository, run, job, holder, attempt, and deadline fields still match.
...
When `delegate-supervisor` sees the same running semantic progress digest for four minutes, it parks the active round and invocation for manual recovery instead of treating repeated polls as fresh progress.
...
Momentum derives the expected external identity only from the current invocation attempt's latest legacy no-mistakes checkpoint or a delegate-supervisor checkpoint whose same-attempt handoff intent selects the no-mistakes tool.

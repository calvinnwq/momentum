...
A valid but unregistered executor identity instead records an attempt and
...
The daemon-dispatchable `external-apply` path uses the same surface when issue scope, tracker evidence, deterministic intent seeding, valid payload, resolved target, credentials, policy, audit, or adapter safety checks refuse the write.
...
2. Confirm the durable intent evidence is either one pending Linear `status_update` intent or deterministic issue-scope/tracker evidence that seeded the expected `Done` status update, with a matching tracker item, stable idempotency marker, and exactly one valid `state` or `stateId` payload.
...
5. If no Linear update landed, the step failed before any external write; treat it like a retryable failure after fixing the missing auth/policy/issue-scope/tracker-evidence/deterministic-intent/payload cause.

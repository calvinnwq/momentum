...
A fallback that proves cleanup preserves the already-known timeout, cancellation, or command-exit outcome; only a fallback that cannot prove cleanup changes the result to `SUPERVISOR_FAILED`.

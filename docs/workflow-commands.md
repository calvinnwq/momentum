...
- **Step detail**: each step carries its `kind`, executor family, optional portable `config`, `order`, `required` flag, and on-start `state` (`pending`, or `approved` for the steps an `--approval-boundary` covers).

...
- **Execution**: `workflow run start` only materializes durable run state.
  ...
  Native coding dispatch reads the frozen per-step harness/model/effort selection from canonical `workflow_steps.agent_config_json` before creating executor work, maps it onto the dispatcher-created round, and routes corrupt canonical step config to manual recovery with `route_config_invalid` instead of silently falling back.

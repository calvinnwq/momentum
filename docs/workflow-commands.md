...
- **Execution**: `workflow run start` only materializes durable run state.
  ...
  An ordinary live-wrapper result is finalized through the shared verify -> commit / reset transaction before terminalization and reconciliation: Momentum reads the agent result's commit intent, writes `verification.log`, commits verified changes, resets safe failures, and attaches the verification log to round evidence.
...
For coding starts, the `Implementation engine` line prints the historical compatibility label recorded for the start; it does not select the active executor.
...
- A `runtime_unavailable` refusal is retryable for any dispatched step after its registered executor, wrapper, credentials, or other runtime dependency is repaired. This includes stale wrapper/build paths, missing no-mistakes branch-start state, current no-mistakes cancellation evidence before clean agent-result evidence exists, and merge-cleanup auth, target, PR readback, expected-head, cleanup-branch, or mergeability refusals reported before clean agent-result evidence exists.
...
Before each round's agent starts, Momentum renders a deterministic prompt from objective, source context, round identity, repo/base-head context, acceptance and verification requirements, prior round evidence, and the configured result path.
Source context and prior-round evidence are quoted as untrusted JSON context, and stale result files are cleared before the agent is asked to write the fresh normalized result JSON.

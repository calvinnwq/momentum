...
Agents may inspect Momentum on native Windows, but process-backed workflow
execution must run on Linux or macOS and otherwise refuses with
`unsupported_platform` before spawn.

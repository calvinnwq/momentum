...
- the `doctor --json` milestone marker reads the M11 closeout string
  (NGX-302 flipped the marker from M5 to M6, NGX-319 from M6 to M7,
  NGX-330 from M7 to M8, NGX-353 from M8 to M10, and NGX-419 from M10
  to M11 — see
...
- the `doctor --json` milestone marker reads the M11 closeout string
  (NGX-302 flipped the marker from M5 to M6, NGX-319 from M6 to M7,
  NGX-330 from M7 to M8, NGX-353 from M8 to M10, then NGX-419 from M10
  to M11).
...
- NGX-499 coding-workflow live-wrapper profile coverage in
  `test/coding-workflow-live-wrapper.test.ts`: the checked-in profile parses,
  covers every live-wrapper-owned coding step, loads
  `MOMENTUM_CODING_WORKFLOW_WRAPPER_CONFIG`, synthesizes successful and failed
  `RunnerResult` evidence, treats missing command config as durable failure
  evidence, and kills timed-out command process trees.
...
- terminalize-and-continue dispatch fixture in
  ...
  `stepNotStartable` / unknown → pass through unchanged —
  `isDogfoodTerminalizeDispatchEnabled` env-var parsing (truthy / falsy / unset),
  and `resolveDaemonWorkflowDispatch` identity passthrough when off versus
  wrapping when opted in.
- single-process multi-dispatch proof in
  `test/workflow-dogfood-multi-dispatch.test.ts` (NGX-391): a single

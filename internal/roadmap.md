...
`src/*.ts` policy, and `ARCH-02` through `ARCH-08` migration order. That
architecture sequence should land before `RC-2` so the M9/M10 step-finalization
reconciliation seam has stable workflow, executor, repo, adapter, and evidence
homes.

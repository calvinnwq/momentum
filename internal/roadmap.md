...
place and tested). The next independent runtime consolidation items are `RC-1`
and the remaining `RC-5` narrowing (wiring those RC-5b seams into the
`daemon start` workflow lane).

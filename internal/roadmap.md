...
3. **NGX-297 — M6-02 Linear external update client** *(this slice)*: introduce the credential-handling Linear GraphQL mutation client behind the adapter boundary. Tests use mock fetch/endpoints; no real `api.linear.app` calls in tests, and the CLI path is still not wired.

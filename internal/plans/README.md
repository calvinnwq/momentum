...
| RC-3 — external-apply dispatch wiring landed (NGX-496) | [`runtime-consolidation-plan.md`](../contracts/runtime-consolidation-plan.md#follow-up-issue-sequence) | Daemon-dispatchable `external-apply` adapter behind M6 safety gates. The pure M6 → executor-evidence mapping (`dispatch-external-apply.ts`), async run-path producer (`dispatch-external-apply-run.ts`), family guard, and daemon dispatch composition reuse the single M6 `executeExternalApply` write path under its full safety contract; `subworkflow` remains fail-closed. |
| RC-4 — adapter mechanism landed (NGX-497); PHASE1 dispatch-lane flip stays deferred | [`runtime-consolidation-plan.md`](../contracts/runtime-consolidation-plan.md#follow-up-issue-sequence) | Daemon-dispatchable `subworkflow` adapter. The pure child-mirror mapping (`dispatch-subworkflow.ts`), the async run-path producer (`dispatch-subworkflow-run.ts`), and the daemon-lane entry-point factory (`subworkflow-dispatch.ts`) ship and are tested behind the parent/child ownership boundary; the production `subworkflow` branch stays fail-closed until a separate PHASE1 dispatch-lane flip lands (deferred pending a child-definition config decision, since no built-in step declares the `subworkflow` family yet). |

...
| RC-3 — adapter landed (NGX-496); fail-closed branch narrowing stays deferred | [`runtime-consolidation-plan.md`](../contracts/runtime-consolidation-plan.md#follow-up-issue-sequence) | Daemon-dispatchable `external-apply` adapter behind M6 safety gates. The pure M6 → executor-evidence mapping (`dispatch-external-apply.ts`) and the async run-path producer (`dispatch-external-apply-run.ts`) ship and reuse the single M6 `executeExternalApply` write path under its full safety contract; the production `external-apply` fail-closed branch in `dispatch.ts` stays in force until a separate narrowing slice switches the dispatch lane. |

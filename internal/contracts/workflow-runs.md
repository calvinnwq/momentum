...
- A stable `WorkflowMonitorRecovery` taxonomy (`stale_running_step`, `ghost_active_no_lease`, `manual_recovery_lease`, `monitor_drift_stale`, `failed_required_step`) and a `needsRecoveryArtifact` flag that follow-up slices use to drive the per-run `recovery.md` artifact and the `WorkflowRun.needs_manual_recovery` durable flag.

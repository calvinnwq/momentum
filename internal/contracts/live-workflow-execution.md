...
Live finalization maps unsafe git / result outcomes into the same run-level taxonomy: moved HEAD becomes `head_mismatch`; missing or untrusted result documents become `result_missing` / `result_invalid`; failed cleanup becomes `reset_failed`; lost ownership becomes `repo_lock_lost`; git and input failures retain `git_failed` and `invalid_input`. Commit failures enter `commit_failed` recovery only when Momentum cannot prove cleanup; `nothing_to_commit` and commit failures followed by a successful reset remain clean step-failure outcomes without the run-scoped recovery flag. Process-level dispatch failures choose the first valid live run-level recovery code from the wrapper's precise `liveRecoveryCode`, then the executor dispatch code, and otherwise fall back to `command_failed`. When the configured wrapper command is `node` (or `/usr/bin/env node`) and the configured script entrypoint itself is missing, that bootstrap failure is `runtime_unavailable`; `MODULE_NOT_FOUND` failures from inside an existing wrapper script remain command failures because the runtime did start. For dispatched `no-mistakes` and `merge-cleanup` steps, an operator clear after such a stale wrapper/build path failure may prepare a scheduler retry on the same invocation. The retry increments the attempt, appends a new executor round, and keeps later-attempt result / log evidence under an attempt-scoped directory so the failed round stays durable and the repaired run does not overwrite it.

...
**Status:** Active. Captures the boundary that landed in M5 (NGX-288..NGX-294) and the read-only invariants that M6 preserved while adding the policy-gated external write path described in [internal/contracts/intent-apply.md](intent-apply.md). M6 is complete; these invariants remain wire-stable.
...
- Adds the NGX-297 Linear external update client behind the adapter boundary. The client owns credential-handling GraphQL writes only: callers supply the preview/comment body/idempotency marker and optional status mutation config. NGX-298 wired this client to the CLI via `intent apply --external-apply`.

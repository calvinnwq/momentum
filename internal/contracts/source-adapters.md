...
- Starts with the NGX-296 `ExternalUpdateAdapter` boundary: registry, input/result types, deterministic dry-run preview, idempotency marker helper, and stable adapter/write error taxonomy with no mutation capability.
  - The stable write-side error taxonomy is `unsupported_adapter`, `unsupported_intent_type`, `target_missing`, `auth_unavailable`, `policy_denied`, `external_conflict`, `adapter_threw`, `write_rejected`, `write_timeout`, `malformed_response`, and `validation_failed`. Reconciliation-specific result codes remain owned by NGX-300/NGX-301 and are intentionally excluded from this adapter taxonomy.
  - A successful dry-run preview returns `ok: true` with a `preview` envelope containing `adapterKind`, `intentId`, `intentType`, `target`, `mutationKind`, `summary`, `commentBody`, and `idempotencyMarker`. Failures return `ok: false`, `code`, and `error`, and the preview dispatcher must not throw.

...
gate held closed. NGX-372 also added the CI-safe decision core for the opt-in
...
wrapper with the external-write family held closed by default.
...
- **NGX-372 — Opt-in real adapter smoke and full E2E proof.** Owns the manual
  ...
  - The CI-safe decision core for the opt-in real **coding-workflow harness**
    ...
    probe-only dry-run. Live execution itself stays coding-workflow-pipeline
    owned (the planner is the gate, not the runner).

  Remaining NGX-372 work: a gated probe-execution test that actually runs the
  resolved harness pre-flight probe when opted in (skipped by default, like
  `test/real-linear-read-smoke.test.ts`), and composing the goal-loop /
  no-mistakes / M9 live-wrapper terminal finalizations into the E2E (each landed
  adapter exists and is isolated-tested, but only the one-shot terminal is
  composed into the proof so far).

...
**Status:** Active audit for NGX-368. This matrix separates adapter families from the
full end-to-end workflow proof so Momentum can harden adapter contracts in
layers: isolated contract tests first, stubbed integration second, opt-in real
smoke third, and full E2E only after the lower layers are green.
...
- **NGX-369 — Isolated source adapter contract tests.** Owns source adapter and
  Linear read-side hardening, especially no-network guards, pagination, response
  shape failures, read-only invariants, and durable reconciliation summaries.

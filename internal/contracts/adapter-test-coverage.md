...
- **NGX-372 — Opt-in real adapter smoke and full E2E proof.** Owns the manual
  ...
  isolated-tested, but only the one-shot terminal is composed into
  `test/full-adapter-e2e.test.ts` so far).

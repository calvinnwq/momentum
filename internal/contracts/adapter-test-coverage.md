...
and goal-loop terminal finalizations with a passing verification gate →
...
- **NGX-372 — Opt-in real adapter smoke and full E2E proof.** Owns the manual
  ...
  finalizations into the E2E (each landed adapter exists and is isolated-tested,
  and the one-shot and goal-loop terminals are now composed into
  `test/full-adapter-e2e.test.ts`; no-mistakes and the live wrapper are not yet).

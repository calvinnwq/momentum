...
| Unsupported executor families: subworkflow; external-apply dispatch adapter | `test/workflow-dispatch-execute.test.ts` covers the durable fail-closed dispatch taxonomy for resolution failures and the remaining unsupported-family branch; `test/workflow-dispatch.test.ts` pins `external-apply` in the dispatchable set and keeps `subworkflow` fail-closed. `test/workflow-dispatch-external-apply.test.ts`, `test/workflow-dispatch-external-apply-run.test.ts`, and `test/workflow-dispatch-external-apply-m6.test.ts` cover the M6 mapping, scaffold-family guard, daemon wrapper reachability, idempotent re-entry, and real `executeExternalApply` reuse through mock Linear clients. | Stubbed dispatch verifies subworkflow/manual-recovery safety; external-apply tests verify terminal executor evidence and fail-closed M6 refusals. | Real external writes remain policy-gated and are mocked in CI; no real api.linear.app calls run in tests. | `test/full-adapter-e2e.test.ts` still covers the historical fail-closed proof, while NGX-496 adds focused external-apply adapter wiring coverage. | Keep subworkflow fail-closed until its future implementation slice lands. |

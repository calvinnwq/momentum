...
goal-loop, and no-mistakes-mirror terminal finalizations with a passing
verification gate (or its mirror equivalent: a corroborated completed external
review gate) → external-write policy gate held closed. NGX-372 also added the CI-safe decision core for the opt-in
real coding-workflow harness smoke (`src/real-workflow-smoke.ts`), the
explicitly-flagged gate for invoking a real GNHF / postflight / no-mistakes
wrapper with the external-write family held closed by default, and the gated
probe-execution layer (`src/real-workflow-probe.ts`,
...
- **NGX-372 — Opt-in real adapter smoke and full E2E proof.** Owns the manual
  ...
  - The CI-safe decision core for the opt-in real **coding-workflow harness**
    smoke: `src/real-workflow-smoke.ts` (`planWorkflowHarnessSmoke` gating over
    ...
    explicitly-flagged opt-in gate for invoking a real GNHF / postflight /
    no-mistakes / merge-cleanup wrapper: skipped unless
  ...
  - The gated **probe-execution** smoke, `test/real-workflow-probe-smoke.test.ts`,
    plus its execution helpers in `src/real-workflow-probe.ts`
    ...
    pure `classifyProbeSpawnResult` mapping in `src/real-workflow-smoke.ts`. The
  ...
  into the E2E (the landed live-wrapper adapter exists and is isolated-tested; the
  one-shot, goal-loop, and no-mistakes-mirror terminals are now composed into
  `test/full-adapter-e2e.test.ts`; the M9 live wrapper is not yet).

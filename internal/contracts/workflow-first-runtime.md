...
- Has M10-04 opt-in daemon workflow scheduler lane (recover -> scan -> claim -> dispatch) that schedules runnable workflow steps alongside goal iteration draining, M10-09a production dispatcher wiring for bounded managed `daemon start`, RC-5b configured daemon-default live-wrapper profile wiring that runs dispatched steps through real commands and feeds terminal executor evidence to the RC-2 reconciliation seam, and NGX-500 retry hardening for stale wrapper bootstrap failures on the same deterministic invocation.

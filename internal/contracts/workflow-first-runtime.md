...
M10 has implemented these as concrete slices: M10-01 landed definition schema / validation / persistence, M10-02 landed CLI run start, M10-03 landed executor state schema / persistence, M10-04 landed the opt-in daemon workflow scheduler lane, M10-05 landed the goal-loop executor adapter, M10-06 landed the one-shot / script executor adapters, M10-07 landed the no-mistakes executor mirror, M10-08 landed durable workflow gates and decisions, M10-09a landed phase-1 production dispatcher wiring, and M10-09 dogfooded the workflow-first start / approval / bounded dispatch path. RC-3 has since landed generalized `external-apply` dispatch through the M6 safety contract; `subworkflow` dispatch remains a later slice.
